...
<div class="card-actions w-100 d-flex justify-content-end gap-2 mt-3">
    <!-- Tombol Balas Vendor -->
    <a href="javascript:void(0)" class="btn primary" onclick="openReplyModal({{ $surat->id_suratadmin }})">
        <i class="fas fa-reply"></i> Balas Vendor
    </a>

    <!-- Tombol Tambah Master Barang -->
    <a href="javascript:void(0)" class="btn btn-success" id="tambahMasterBarangBtn">
...
<script>
    $('#tambahMasterBarangBtn').on('click', function(){
    Swal.fire({
        icon: 'warning',
        title: 'Perhatian!',
        html: "Sebelum menambahkan <b>MASTER BARANG</b>, harap membalas surat kontrak terlebih dahulu agar bisa melanjutkan ke tahap berikutnya.",
        showCancelButton: true,
        confirmButtonText: 'Balas',
        cancelButtonText: 'Lanjut Tambah Master Barang',
        allowOutsideClick: false,
        allowEscapeKey: false
    }).then((result) => {
        if(result.isConfirmed){
            // Jika pilih "Balas", buka modal balas
            openReplyModal({{ $surat->id_suratadmin }});
        } else {
            // Jika pilih "Lanjut Tambah Master Barang", redirect ke halaman tambah master barang
            window.location.href = "{{ route('masterbarang.index') }}";
        }
    });
});
</script>
