...
<div class="card-actions d-flex gap-2">
    <!-- Tombol Balas -->
    <a href="javascript:void(0)" class="btn primary" id="btnBalas">
        <i class="fas fa-reply"></i> Balas
    </a>

    <!-- Tombol Tambah Pengiriman -->
    <a href="{{ route('vendor.pengiriman') }}" class="btn btn-success">
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content" style="border-radius: 12px; border: none;">
            <div class="modal-header border-0">
                <h4 class="m-0 font-weight-bold">Unggah Dokumen Barang</h4>
...
<script> 
    ...
    Swal.fire({
        icon: 'info',
        title: 'Perhatian',
        text: 'Jika ingin membalas surat, lakukan PENGIRIMAN terlebih dahulu, agar lebih mudah melanjutkan ke tahap berikutnya.',
        showCancelButton: true,
        confirmButtonText: 'Lanjut Balas',
        cancelButtonText: 'Tambah Pengiriman',
        allowOutsideClick: false,
        allowEscapeKey: false
    }).then((result) => {
        if(result.isConfirmed){
            // Lanjutkan ke modal balas
            openReplyModal({{ $surat->id_surat }});
